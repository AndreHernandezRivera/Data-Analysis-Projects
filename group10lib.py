...
def preprocess_dataset(dataset_path="big_five_scores.csv",num_feats=3 ):
    ...
    numcols = ['case_id', 'age', 'agreeable_score','extraversion_score', 'openness_score', 'conscientiousness_score',
                'neuroticism_score']
    catcols = ['country','sex']
    ...
    y = traindf['country_USA']
    X = traindf.copy()
    
    ### remove all dummy countries not the target dummy country
    x_cols=[ x for x in features if "country_" not in x ]
    X=X[x_cols]
    print(X)
    #del X['country']
...
best_features,X,y = autoFeatureSelector(dataset_path="big_five_scores.csv", methods=['pearson', 'chi-square', 'rfe', 'log-reg', 'rf', 'lgbm'])
print(best_features)
